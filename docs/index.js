...
console.log(reverseString("!eMeriH"));
...
console.log(isPalindrome('somos')); // true
console.log(isPalindrome('palindromo')); // false
...
function closestPair(arr) {
  // Orden del array de menor a mayor
    arr.sort((a, b) => a - b);

    let minDiff = Infinity; // Inicializamos la diferencia mínima como infinita
    let pair = []; // Array para almacenar el par con la menor diferencia

    // Recorremos el array para comparar pares adyacentes
    for (let i = 0; i < arr.length - 1; i++) {
        const diff = arr[i + 1] - arr[i]; // Diferencia entre el elemento actual y el siguiente
        if (diff < minDiff) {
            minDiff = diff; // Actualizamos la menor diferencia
            pair = [arr[i], arr[i + 1]]; // Guardamos el par de números
        }
    }

    return pair;
}
console.log(closestPair([4, 2, 1, 7, 9, 10])); // [1, 2]
console.log(closestPair([1, 5, 9, 12, 15]));   // [9, 12]
